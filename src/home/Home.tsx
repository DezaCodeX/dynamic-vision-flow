...
export function Home() {
  ...
  return <><Cursor /><Navbar onBook={openBooking} /><div className={`fixed inset-x-0 top-[7rem] z-40 transition-all duration-300 ${bookingBarVisible ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-[200%] opacity-0"}`}><BookingWidget onBook={openBooking} /></div><main ref={root} className="relative pt-[13rem] sm:pt-[12rem] md:pt-[11rem] lg:pt-[10rem]"><Hero onBook={openBooking} /><AboutPreview /><RoomsPreview /><DiningPreview /><Amenities /><Experience /><GalleryPreview /><Testimonials /><LocationPreview /><BookingCTA onBook={openBooking} /><Footer onBook={openBooking} /></main><BookingPanel open={booking} onClose={() => setBooking(false)} /></>;
}
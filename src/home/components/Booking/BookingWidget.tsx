export function BookingWidget({ onBook }: { onBook: () => void }) {
  return <section className="relative z-10 mx-auto max-w-[1200px] px-6 lg:px-12" data-home-reveal>
    <form onSubmit={(event) => { event.preventDefault(); onBook(); }} className="glass-panel grid grid-cols-2 gap-5 p-6 shadow-[var(--shadow-cinema)] lg:grid-cols-[1fr_1fr_0.8fr_0.8fr_auto] lg:items-end lg:p-8">
      ...
      <label><span className="eyebrow">Rooms</span><select className="mt-3 w-full border-b border-input bg-transparent pb-2 text-sm text-cream outline-none"><option>1</option><option>2</option><option>3</option><option>4+</option></select></label>
      <button className="col-span-2 border border-gold bg-gold px-7 py-3 text-[0.6rem] tracking-[0.35em] text-primary-foreground uppercase transition-colors hover:bg-transparent hover:text-gold lg:col-span-1">Book now</button>
    </form>
  </section>;
}
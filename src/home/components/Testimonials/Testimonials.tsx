...
export function Testimonials({ reviews = demoReviews }: { reviews?: GuestReview[] }) {
  ...
  return (
    <section className="bg-[var(--color-surface-blue-soft)] px-6 py-24 lg:px-12 lg:py-32">
      <div className="mx-auto max-w-[1400px]">
        ...
        <div
          className="review-carousel mt-12"
          tabIndex={0}
          ...
          {reviews.map((review, index) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 font-display text-2xl leading-tight text-ink">{review.text}</p>
        ...
        <div className="mt-6 flex gap-2">
          {reviews.map((review, index) => (
            <button key={`${review.guest}-dot`} type="button" onClick={() => setActiveIndex(index)} aria-label={`Show review ${index + 1}`} aria-current={activeIndex === index} className={`size-2 rounded-full border border-gold ${activeIndex === index ? "bg-gold" : "bg-transparent"}`} />
          ))}
        </div>
      </div>
    </section>
